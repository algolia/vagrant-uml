    ...
    module Action
      class Create
        ...
        def call(env)
          ...
          @cli.create_cidata(:root_path => env[:machine].env.root_path.to_s , :name => env[:machine].name , :mac => env[:machine].provider_config.mac, :data_dir => env[:machine].data_dir.to_s)
          @app.call(env)
        end
      end
    end
